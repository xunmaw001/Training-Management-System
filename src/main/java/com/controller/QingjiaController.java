...
, "laoshiId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 老师
            //级联表
            LaoshiEntity laoshi = laoshiService.selectById(qingjia.getLaoshiId());
            if(laoshi != null){
    ...
    */
    ...
    public R save(@RequestBody QingjiaEntity qingjia, HttpServletRequest request){
        ...
        if(false)
            ...
            qingjia.setLaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<QingjiaEntity> queryWrapper = new EntityWrapper<QingjiaEntity>()
            .eq("yonghu_id", qingjia.getYonghuId())
            .eq("laoshi_id", qingjia.getLaoshiId())
...
//                            qingjiaEntity.setLaoshiId(Integer.valueOf(data.get(0)));   //老师 要改的
