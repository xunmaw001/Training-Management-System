    ...
    */
    ...
    public R save(@RequestBody ZuoyeEntity zuoye, HttpServletRequest request){
        ...
        Wrapper<ZuoyeEntity> queryWrapper = new EntityWrapper<ZuoyeEntity>()
            ...
            .eq("zuoye_address", zuoye.getZuoyeAddress())
            .eq("zuoye_types", zuoye.getZuoyeTypes())
            .eq("zuoye_video", zuoye.getZuoyeVideo())
            .eq("zuoye_delete", 1)
            ;
...
//            zuoye.setLaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        if("".equals(zuoye.getZuoyeVideo()) || "null".equals(zuoye.getZuoyeVideo())){
                zuoye.setZuoyeVideo(null);
        }
...
//                            zuoyeEntity.setZuoyeName(data.get(0));                    //实训模块名称 要改的
//                            zuoyeEntity.setZuoyeUuidNumber(data.get(0));                    //实训模块编号 要改的
//                            zuoyeEntity.setZuoyeAddress(data.get(0));                    //实训模块地点 要改的
//                            zuoyeEntity.setZuoyePhoto("");//详情和图片
//                            zuoyeEntity.setZuoyeTypes(Integer.valueOf(data.get(0)));   //实训模块类型 要改的
//                            zuoyeEntity.setZuoyeVideo(data.get(0));                    //视频 要改的
//                            zuoyeEntity.setZuoyeFile(data.get(0));                    //文件 要改的
...
//                            zuoyeEntity.setCreateTime(date);//时间
                        ...
                        if(zuoyeEntities_zuoyeUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [实训模块编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
